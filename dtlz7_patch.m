...
%% DTLZ7
Tbf = readtable('dtlz7_nsga3.csv');  
df = table2array(Tbf); dataf = df(2:end,2:14);

%% local
Tb = readtable('dtlz7_patch2.csv');  
d = table2array(Tb); d = d(d(:,4)<5.0 & d(:,2)>0.4 & d(:,3)>0.4,:);
data = d(2:end,2:14); F(:,1) = data(:,1); F(:,2) = data(:,2); F(:,3) = data(:,3);
Tb1 = readtable('dtlz7_alter.xlsx');
d1 = table2array(Tb1);
alter = reshape(d1,[3,17])';
post_cls = [0.7173886 0.7173908 3.901683; 0.6679063 0.6679096 4.645172];

%% Plot the solution points.
figure(1)
scatter3(F(:,1),F(:,2),F(:,3),10,'bo','filled'); 
hold on;
scatter3(dataf(:,1),dataf(:,2),dataf(:,3),3,'ko','filled');
scatter3(alter(:,1),alter(:,2),alter(:,3),15,'go','filled');
scatter3(post_cls(:,1),post_cls(:,2),post_cls(:,3),20,'ms','filled');
scatter3(alter(9,1),alter(9,2),alter(9,3),20,'ro','filled');
legend('Pareto Front','NSGA III','Alter Soln','Inter Soln','fin soln','Location','best')
xlabel('F1')
ylabel('F2')
zlabel('F3')
...
[sMap_F]= som_lininit(sData_F,'lattice','hexa','msize',[22,22]);
[sMap]= modifiedsom_lininit1(sData,'lattice','hexa','msize',[22,22]);
